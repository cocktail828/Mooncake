...
class WeightedRandomAllocationStrategy : public RandomAllocationStrategy {
   private:
    static constexpr uint64_t kWeightShift = 20;          // 1MB
    static constexpr uint64_t kWeightMax = 1ULL << 30;  // 10GB

   public:
    WeightedRandomAllocationStrategy() = default;
    virtual ~WeightedRandomAllocationStrategy() = default;

    // https://en.wikipedia.org/wiki/Fitness_proportionate_selection, Weighted
    // Random Selection
    virtual std::vector<std::unique_ptr<AllocatedBuffer>> allocateBatch(
        const AllocatorManager& allocator_manager, const size_t slice_length,
        std::mt19937& generator, const size_t expect_num,
        ...
        for (const auto& name : names) {
            ...
            for (const auto& alloc : *allocators) {
                ...
                if (avail > slice_length) {
                    uint64_t weight = avail >> kWeightShift;
                    // If weight is larger than kWeightMax, it does not matter how much 
                    // space it occupies, we just cap it to kWeightMax.
                    if (weight > kWeightMax) weight = kWeightMax;
                    total_weight += weight;
                    candidates.push_back({alloc, weight, total_weight, false});
                }
            }
        }
        ...
        // Step 3: Selection loop (without replacement)
        std::uniform_real_distribution<double> dist(0.0, total_weight);
        for (size_t i = 0; i < expect_num && !candidates.empty(); ++i) {
